...
var DEFAULT_INSTRUCTION_TO_FIX;
...
//                if(!instruction || instruction == undefined)throw new Error('bixcoito');
//                console.log('register activate ',instruction);
                if(instruction == undefined){
                    instruction = DEFAULT_INSTRUCTION_TO_FIX;
                }else if(instruction.component == 'splash'){
//                    console.log('e é splash.. hummmmm');
                    DEFAULT_INSTRUCTION_TO_FIX = instruction;
                }
...
//        if(!instruction || instruction == undefined)return false;
...
function loadTemplatesStepFactory($componentMapper, $templateRequest) {
    return function loadTemplates(instruction) {
        return instruction.router.traverseInstruction(instruction, function (instruction) {
//            console.log('call component to template ',instruction.component);
...
function $componentMapperFactory($location) {
    ...
    var componentToTemplate = function componentToTemplateDefault(name) {
        var dashName = snakeCase(name);
        return './views/' + dashName + '/' + dashName + '.html';
    };
...
var batatinhaquandonascejacaranaotempescocosetumeamapqrouboumeupatinete;
angular.module('ngNewRouter').factory('$$rootRouter', ['$q', '$$grammar', '$$pipeline', function ($q, $$grammar, $$pipeline) {
    ...
    (createClass)(Router, {
        ...
        navigate: function (url) {
            ...
            batatinhaquandonascejacaranaotempescocosetumeamapqrouboumeupatinete = url;
...
//                console.log('then ',instruction);
    ...
    function mapObj(obj, fn) {
        ...
        Object.keys(obj).forEach((function (key) {
//            console.log(obj[key]);
//            if(obj[key].component != undefined){
...
angular.module('ngNewRouter').factory('$$grammar', ['$q', function ($q) {
    ...
    (createClass)(Grammar, {
        ...
        recognize: function (url, mapping) {
            ...
            if (lastParams && lastParams.childRoute) {
                ...
                if(instruction.outlets[Object.keys(instruction.outlets)[0]]==undefined){
                    var i = 0, len = _mapping.length, myUrl;
                    
                    for(i; i < len;i++){
                        myUrl = _mapping[i].otherwiseRedirectTo;
                        if(myUrl != undefined){
                            break;
                        }
                    }
                    instruction.canonicalUrl = myUrl;
                    forEach(lastHandler.components, (function (componentName, outletName) {
                        instruction.outlets[outletName] = {
                            outlets: {}
                        };
                    }));
                }else{
